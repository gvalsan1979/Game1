namespace Assets.Scripts {

    public class Scene3_BeachConstants {

        public const float BEACH_BORDER_Y = -2.5f;

        public const float SENDOFF_LEFT_X = -13.0f;

        public const float SENDOFF_RIGHT_X = 13.0f;

        public const float ARRIVE_LEFT_X = -6.5f;

        public const float ARRIVE_RIGHT_X = 7.5f;


        public const float SENDOFF_Y = 7.0f;

        public const float TIME_PER_PATH = 3.0f;

        public const float ENEMIES_PER_SECOND = 0.5f;

        public const float COIN_PER_SECOND = 0.3f;

        public const float COIN_LIFETIME = 2.0f;

        public const float AMPLITUDE_SPATH = 1.5f;

        public const float BOMB_LEVEL_TIME = 30.0f;
    }
}
